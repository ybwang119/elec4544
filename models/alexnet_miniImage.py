...
import torch
class Alexnet_miniImage(nn.Module):
    def __init__(self,classes: int):
        super().__init__()
        self.conv1=nn.Conv2d(in_channels=3,out_channels=96,kernel_size=11,stride=2,padding=2)
        self.relu=nn.ReLU(inplace=True)
        self.maxpool1=nn.MaxPool2d(kernel_size=3,stride=2)
        self.conv2=nn.Conv2d(in_channels=96,out_channels=256,stride=1,padding=2,kernel_size=5)
        self.conv3=nn.Conv2d(in_channels=256,out_channels=384,stride=1,padding=1,kernel_size=3)
        self.conv4=nn.Conv2d(in_channels=384,out_channels=384,padding=1,stride=1,kernel_size=3)
